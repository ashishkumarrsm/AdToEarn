...
exports.addDeposite = catchAsyncErrors(async (req, res, next) => {
  let { amount, user_id ,hash} = req.body;
  console.log(amount, user_id ,hash)
    const insertTopupSql = `INSERT INTO user_deposite (user_id, amount , currency,hash) VALUES (?, ?, ?,?)`;
    db.query(insertTopupSql, [user_id, amount,"BEB20",hash], (err, insertResult) => {
      if (err) {
...
//   const uploadDir = path.join(__dirname, '..', '..', 'finrain.info', 'uploads');
